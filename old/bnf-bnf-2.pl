...
test :-
	read_data_from_file('/var/lib/myfrdcsa/codebases/minor/universal-parser/languages/bnf/bash-2.0.bnf',Data),
	atom_codes('-5*(73+7)',Codes),
	phrase(parse(Output),Codes,Rest),
	length(Rest,LengthRest),
	length(Codes,LengthCodes),
	LengthPrefix is LengthCodes - LengthRest,
	append([A,B],Codes),
	length(A,LengthPrefix),
	atom_codes(ItemAtom,A),
	view([outputs,Output,itemAtom,ItemAtom,rest,Rest]),
	fail.
test.
	%% view([x,X]).